...
const REPO_PATH =
	/^(?:\.\.\/)*(?:LICENSE|README\.md|RELEASE_NOTES\.md|auto\/|config\/|packages\/|tools\/|repo\/|docs\/(?!\d))/;
...
function main() {
	...
	for (const file of files) {
		...
		const destName = file; // keep .md
		fs.writeFileSync(path.join(DEST, destName), out);
		console.log(`wrote docs/${destName}`);
	}
...
`;
	...
	console.log(`synced ${files.length} docs → ${path.relative(ROOT, DEST)} (${linkCount} /docs/ links)`);
}
